...
import { buildCountySplits } from './build-county-splits.mjs';
...
// County-exact two-tone fills for the states a zone boundary crosses.
const { result: countySplits, audit: splitAudit } = buildCountySplits(toPath);
for (const shape of shapes) {
  const parts = shape.usps ? countySplits[shape.usps] : null;
  if (parts) shape.parts = parts;
}
...
console.log(`  county-exact splits: ${shapes.filter((x) => x.parts).length}/${shapes.filter((x) => x.split).length} split states`);
for (const line of splitAudit) console.log('    ' + line);
